...
import { Eye, EyeOff, Mail, Lock, User, School, Briefcase, Phone, Loader2, AlertCircle, CheckCircle2, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const [step, setStep] = useState(1); // 1 = account, 2 = school info
  ...
  const [form, setForm] = useState({
    ...
    schoolName: '', schoolLocation: '', designation: '', className: '', mobile: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { token, user } = await apiSignup({
        ...
        schoolName: form.schoolName, schoolLocation: form.schoolLocation,
        designation: form.designation, className: form.className, mobile: form.mobile,
      });
      setAuth(token, user);
      setProfile({
        name: user.name, email: user.email, mobile: user.mobile || '',
        schoolName: user.schoolName || '', schoolLocation: user.schoolLocation || '',
        designation: user.designation || '', className: user.className || '',
        avatar: user.avatar || '',
      });
      router.push('/');
    } catch (err: any) {
          ...
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all ${
                step > s ? 'bg-emerald-500 text-white' : step === s ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-500'
              }`}>
              ...
              {s < 2 && <div className={`flex-1 h-px ${step > s ? 'bg-emerald-500' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={form.email} onChange={e => update('email', e.target.value)}
                    placeholder="teacher@school.edu" className={inputClass} autoComplete="email" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => update('password', e.target.value)}
                    placeholder="Min. 6 characters" className={`${inputClass} pr-10`} autoComplete="new-password" />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" value={form.confirmPassword} onChange={e => update('confirmPassword', e.target.value)}
                    placeholder="Repeat password" className={inputClass} autoComplete="new-password" />
                </div>
              </div>
              <button type="button" onClick={handleNext}
                className="w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-800 text-white font-semibold py-3 rounded-xl transition-all mt-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={form.schoolName} onChange={e => update('schoolName', e.target.value)}
                    placeholder="Delhi Public School" className={inputClass} />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <School className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  <input type="text" value={form.schoolLocation} onChange={e => update('schoolLocation', e.target.value)}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={form.designation} onChange={e => update('designation', e.target.value)}
                      placeholder="Teacher" className={inputClass} />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" value={form.mobile} onChange={e => update('mobile', e.target.value)}
                      placeholder="+91 98765..." className={inputClass} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3 mt-2">
                <button type="button" onClick={() => { setStep(1); setError(''); }}
                  className="flex items-center gap-1.5 border border-gray-200 text-gray-700 font-semibold py-3 px-5 rounded-xl hover:bg-gray-50 transition-all">
                  <ChevronLeft className="w-4 h-4" /> Back
                </button>
                <button type="submit" disabled={loading}
                  className="flex-1 flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-800 disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition-all">
